...
export function TutorBookingCta({ tutor }: { tutor: Tutor }) {
  ...
  return (
    <div className="lg:sticky lg:top-28">
      <div className="rounded-[1.75rem] border border-ink/10 bg-cream p-6 shadow-xl shadow-ink/10">
        <div className="flex items-center gap-3">
          <div className="h-14 w-14 overflow-hidden rounded-full">
            <TutorAvatar tutor={tutor} className="h-full w-full" sizes="56px" textClassName="text-lg" />
          </div>
          <div>
            <p className="font-display text-lg text-ink">{tutor.name}</p>
        ...
        <button
          ...
          className="mt-6 w-full rounded-full bg-forest px-6 py-3.5 text-sm font-medium text-cream transition-colors hover:bg-forest-deep"
        >
          {dict.trial.cta}
        </button>
      </div>
