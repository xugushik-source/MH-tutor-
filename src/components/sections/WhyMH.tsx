    ...
    <section className="bg-cream py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 sm:px-8">
        <SectionReveal className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-4 font-display text-4xl text-espresso sm:text-5xl">{dict.why.title}</h2>
        </SectionReveal>
        ...
        <StaggerReveal className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {benefits.map((benefit, i) => {
            ...
            return (
              <div
                key={benefit.title}
                className="flex flex-col gap-4 rounded-[1.5rem] border border-espresso/10 bg-cream-dim/40 p-7 transition-colors duration-300 hover:border-burgundy/30"
              >
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-burgundy/10 text-burgundy">
                  <Icon className="h-5 w-5" />
                </div>
                <h3 className="font-display text-xl text-espresso">{benefit.title}</h3>
                <p className="text-sm leading-relaxed text-espresso/60">{benefit.description}</p>
              </div>
            );
          })}
        </StaggerReveal>
